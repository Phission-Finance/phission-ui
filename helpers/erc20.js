...
export async function withdraw(contract, amount, callStatic) {
    ...
    let ethContract = new ethers.Contract(contract.address, contract.abi, signer);
...
export async function zapBuy(amountIn, minAmountOut, future, ether, callStatic) {
    ...
    const ethContract = new ethers.Contract(zapContract.address, zapContract.abi, signer);
...
export async function zapStakeLP(amountIn, minAmountOut, future, callStatic) {
    const ethereum = window.ethereum;
...
export async function zapStakeLP2(amountIn, minAmountOut, callStatic) {
    const ethereum = window.ethereum;
...
export async function wethMint(amountIn) {
    ...
    const ethContract = new ethers.Contract(wethSplitContract.address, wethSplitContract.abi, signer);
...
export async function lpMint(amountIn) {
    ...
    const ethContract = new ethers.Contract(lpSplitContract.address, lpSplitContract.abi, signer);
    return await ethContract.mint(amountIn);
}
...
export async function phiMint(amountIn) {
    ...
    const ethContract = new ethers.Contract(phiSplitContract.address, phiSplitContract.abi, signer);
    return await ethContract.mint(amountIn);
}
...
export async function swapExactTokensForTokens(contract, amountIn, minAmountOut, from, to, deadline, callStatic) {
    ...
    if (callStatic) {
        return await ethContract.callStatic.swapExactTokensForTokens(
            amountIn,
            minAmountOut,
            [from, to],
            signer.getAddress(),
        ...
        return await ethContract.swapExactTokensForTokens(
            amountIn,
            minAmountOut,
            [from, to],
            signer.getAddress(),
...
export async function swapExactETHForTokens(contract, amountIn, minAmountOut, to, deadline, callStatic) {
    ...
    if (callStatic) {
        return await ethContract.callStatic.swapExactETHForTokens(
            minAmountOut,
            [weth.address, to],
            signer.getAddress(),
            Math.floor(Date.now() / 1000) + deadline,
        ...
        return await ethContract.swapExactETHForTokens(
            minAmountOut,
            [weth.address, to],
            signer.getAddress(),
            Math.floor(Date.now() / 1000) + deadline,
...
export async function chainlinkLatestAnswer() {
    ...
    const ethContract = new ethers.Contract(chainlinkEthUsd.address, chainlinkEthUsd.abi, provider);
